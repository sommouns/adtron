  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="72px">
      <el-form-item>
        ...
        <el-form-item label="暂停状态" prop="type">
          <el-select
        ...
        <el-form-item label="不满21天" prop="isDay">
          <el-select
    ...
    <el-table v-loading="loading" :data="tableList" :row-class-name="tableRowClassName"
      ...
      <el-table-column label="关键词是否暂停" align="center" prop="state" width="130">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.sys_lab_stop_yes_no" :value="scope.row.state"/>
        </template>
      </el-table-column>
      <el-table-column label="满21天" align="center" prop="isDay" width="130">
        <template slot-scope="scope">
...
<script>
import { labKeywordStopList,batchLabKeywordStop} from "@/api/amazon/campaignsinfo";
import { allAccount } from "@/api/amazon/account";
export default {
  name: "labKeywordStop",
  dicts: ['sys_show_hide', 'sys_lab_stop_yes_no'],
  data() {
    return {
      pickerOptions: {
        disabledDate(time) {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      labKeywordStopList(this.queryParams).then(response => {
    ...
    submitFormState(){
      ...
      batchLabKeywordStop(this.checkBoxData).then((response) => {
