    ...
    <el-table v-loading="loading" :data="tableList" height="650">
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <dict-tag :options="dict.type.sys_lab_negative_yes_no" :value="scope.row.status"/>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      labKeywordTargetList(this.queryParams).then((response) => {
    ...
    submitFormState() {
      addKeywordTarget(this.querysId).then((response) => {
