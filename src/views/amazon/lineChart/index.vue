  ...
  <div class="app-container">
    ...
    >
      ...
      <div class="flexx" title="统计信息">
        ...
        <div class="flexy">
          <span>自然订单</span
          ><span>{{ chartTotalResponseDto.naturalOrder }}</span>
        </div>
        <div class="flexy">
          <span>自然销售额</span
          ><span>{{ chartTotalResponseDto.naturalFees }}</span>
        </div>
      </div>
    </el-form>
    <el-form> </el-form>
...
import { selectData } from "@/api/amazon/lineChart";
...
export default {
  ...
  methods: {
    ...
    selectAccount() {
      ...
      accountCampaign(this.queryParams.shopId).then((response) => {
    ...
    selectCampaign() {
      ...
      this.queryParams.asin = undefined;
      if (
        this.queryParams.campaignId == null ||
        this.queryParams.campaignId == undefined ||
        this.queryParams.campaignId == ""
      ) {
        return;
      }
      campaignAsin(this.queryParams.campaignId).then((response) => {
    ...
    modifyData() {
      ...
      this.loading=true;
      selectData(this.queryParams).then((response) => {
        this.loading = false;
