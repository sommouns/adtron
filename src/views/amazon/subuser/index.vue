      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['system:user:add']"
            v-if="
              scope.row.roles[0].roleId == 2 || scope.row.roles[0].roleId == 3
            "
            >添加下级</el-button
          >
          <el-button
            ...
            v-hasPermi="['system:user:add']"
            v-if="scope.row.roles[0].roleId == 4"
            >运营ASIN</el-button
          >
          <el-button
            size="mini"
...
export default {
  ...
  methods: {
    getList() {
      this.loading = true;
      listsubuser(this.queryParams).then((response) => {
    ...
    handleChange(value, direction, movedKeys) {
      this.asinParams.asins = movedKeys;
      //增加asin
      if (direction == "right") {
        saveasin(this.asinParams).then((response) => {
          console.log(response);
        });
      } else if (direction == "left") {
        //删除asin
        delasin(this.asinParams).then((response) => {
          console.log(response);
        });
      }
    },
    handleAsin(row) {
      getselectasin(row.userId).then((response) => {
        ...
        this.asinselect = response.selected;
        //设置要操作的用户
        this.asinParams.userId = row.userId;
        this.asinopen = true;
      });
    },
    queryAsin(row) {
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.userId != undefined) {
            ...
            addsubuser(this.form).then((response) => {
