<template>
  <div class="app-container" style="margin-left: 50px">
    ...
    >
      <el-form-item label="adGroupId">
        <el-input v-model="bidRequest.adGroupId" placeholder="adGroupId"></el-input>
      </el-form-item>
...
<script>
import { spTargetBid } from "@/api/amazon/bidRecommendatios";
export default {
  name: "spTargetBid",
  components: {},
  data() {
  ...
  methods: {
    suggestBid() {
      if (this.bidRequest.adGroupId == undefined) {
        this.$modal.msgError("请输入广告组ID");
        return false;
      }
      ...
      spTargetBid(this.bidRequest).then((response) => {
